...
export default function Topbar() {
  ...
  const { user, profile, clearAuth, toggleSidebar } = useAppStore();
        ...
        <div className="relative" ref={menuRef}>
          ...
          >
            ...
            <div className="hidden md:flex flex-col text-left">
              <span className="text-xs font-bold text-gray-700 leading-tight">{user?.email?.split('@')[0] || "User"}</span>
              <span className="text-[10px] text-gray-400 font-medium">
                {profile?.role === 'teacher' ? 'Instructor' : profile?.role === 'student' ? 'Student' : 'User'}
              </span>
            </div>
