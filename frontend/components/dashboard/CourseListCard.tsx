import React from 'react';
import { ChevronLeft, ChevronRight, ClipboardList } from 'lucide-react';

interface Course {
  id: string;
  title: string;
  level: string;
  modulesCompleted: number;
  totalModules: number;
  percentage: number;
  mentorName: string;
  mentorImage?: string;
  color: 'white' | 'orange' | 'purple'; // Just distinct styles if needed
}

const CourseListCard: React.FC<{ items: Course[]; title?: string }> = ({ 
  items,
  title = "Your courses"
}) => {
  return (
    <div className="bg-[#FF6B6B] rounded-[2rem] p-8 text-white relative overflow-hidden h-auto md:h-[550px] flex flex-col">
       {/* Header */}
       <div className="flex items-center justify-between mb-8 relative z-10 flex-nowrap">
          <h2 className="text-2xl md:text-3xl font-medium truncate mr-2">{title}</h2>
          
          <div className="flex items-center gap-4 flex-shrink-0">

             <button className="bg-white text-[#FF6B6B] px-5 py-2 rounded-full font-medium text-sm hover:bg-gray-50 transition-colors whitespace-nowrap">
                View all
             </button>
          </div>
       </div>

       {/* Cards List */}
       <div className="flex flex-col gap-4 relative z-10 md:overflow-y-auto pr-0 md:pr-2 thin-scrollbar">
          {items.map((course, index) => (
             <div key={course.id} className={`bg-white rounded-[1.5rem] p-5 text-gray-800 shadow-sm flex flex-col justify-between ${index >= 3 ? 'hidden md:flex' : ''}`}>
                <div>
                  <h3 className="text-xl font-bold mb-3">{course.title}</h3>
                  <div className="flex items-center gap-2 text-xs font-semibold text-gray-500 mb-6">
                     <ClipboardList size={16} className="text-orange-500" />
                     <span>{course.modulesCompleted}/{course.totalModules} modules</span>
                  </div>
                </div>

                <div>
                   {/* Progress removed */}
                   {/* 
                   <div className="flex items-center justify-between text-sm font-bold mb-1">
                      <span>{course.percentage}% completed</span>
                   </div>
                   <div className="h-1.5 w-full bg-gray-100 rounded-full mb-4">
                      <div className="h-full bg-orange-500 rounded-full" style={{ width: `${course.percentage}%` }}></div>
                   </div> 
                   */}

                   <div className="flex items-center gap-2">
                      <img 
                        src={course.mentorImage || `https://ui-avatars.com/api/?name=${course.mentorName}`} 
                        alt="Mentor" 
                        className="w-8 h-8 rounded-lg object-cover" 
                      />
                      <div className="flex flex-col leading-tight">
                         <span className="text-[10px] text-gray-400 font-semibold uppercase">Mentor</span>
                         <span className="text-xs font-bold">{course.mentorName}</span>
                      </div>
                   </div>
                </div>
             </div>
          ))}
       </div>
    </div>
  );
};

export default CourseListCard;
