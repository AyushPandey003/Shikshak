 ...
 * @param {string} params.notes_id - Video ID (from database).
 * @param {string} params.blob_name - Azure Blob Name (azure_id).
 */
export const ingestNotesToRag = async ({ course_id, module_id, notes_id, blob_name }) => {
    try {
        console.log(`[RAG-CLIENT] Starting ingestion for video ${notes_id}`);
